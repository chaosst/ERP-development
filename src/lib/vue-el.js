...
var elementTemp = {
  ...
  dateDom : {
    ...
    template:'<el-date-picker :style="{width:opt.width,height:opt.height}" v-model="mytime" @blur="blur" @focus="focus" @change="updateValue" :readonly="opt.readonly" :disabled="opt.disabled" :editable="opt.editable" :type="opt.type" :placeholder="opt.placeholder" :clearable="opt.clearable" :size="opt.size?opt.size:\'small\'" :format="opt.format" :popper-class="opt.popperClass" :picker-options="opt.pickerOptions" :default-value="opt.defaultValue" :clear-icon="opt.clearIcon" :prefix-icon="opt.prefixIcon" :value-format="opt.valueFormat" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期"></el-date-picker>'
  },
  //级联下拉选择框封装
  cascaderDom : {
    data:function(){
  ...
  tableFormDom : {
    ...
    template:'<f-form :model="scope.row" :class="\'row\'+scope.$index+\'-\'+prop+\' fant-table-form\'" :opt="{rules:rules,inlineMessage:true}"><el-form-item :prop="prop"><slot></slot></el-form-item></f-form>'
  },
  //table封装
  tableDom : {
    data:function(){
