    ...
    <el-popover
      ...
      <f-form ref="loginForm" :model="formData" :opt="formOpt" v-enter>
        ...
        <f-form-item class="inputItem" :opt="{label:'采购员：',prop:'inMemberId'}">
          <f-select :opt="selectOpt" v-model="formData.inMemberId" @visible-change="myclose"></f-select>
        </f-form-item>
        <div>
...
//          url:"member/vagueSearch",
    ...
    methods:{
      ...
      changepage(index){
        Vue.http.post("warehouse/cgrk/list",{page:index}).then(function(re){
          if(re.body.rows){
      ...
      delRow(){
        ...
        this.$http.post("warehouse/cgrk/delete",{id:this.fid}).then(function(re){
          if(re.body.returnCode == 0){
            this.alert({msg:"删除成功！"});
            Vue.http.get("warehouse/cgrk/list").then(function(re){
              if(re.body.rows){
      ...
      searchTable:function(){
        this.$refs["loginForm"].$children[0].validate(function(valid){
          if (valid) {
            this.$refs["aaa"].reloadGrid(this.formData);
          } else {
            return false;
          }
        });
      ...
      tbSelect:function(row, oldrow){
        ...
        Vue.set(this.disBtn,"del",a.del);
        Vue.set(this.disBtn,"cancel",a.cancel);
        Vue.set(this.disBtn,"copy",a.copy);
        Vue.set(this.disBtn,"audit",a.audit);
        this.fid = row.fid;
      },
